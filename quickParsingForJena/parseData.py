...
for ch in ["00","01","02","03","04","05","06","07","08","09","10"]:
    ...
    dataFile = 'quickData/' + dataSet + '/raw-ch'+ ch +'.dat'
    ...
    if peak and args.adc:
        ...
        plt.figure()
        plt.title('peak max diagram, channel no. '+ch)
        ...
        plt.savefig(outPath+'/peakDiag_ch'+ch+'.pdf', format="pdf", dpi=400)
        info = open(outPath+'/info_ch'+ch+'.txt', 'w')                      #verbesserbar
        info.write(infostr)
        info.close()
        vals = np.asarray(peak)
        np.savetxt(outPath+'/peakDiag_ch'+ch+'.csv', vals, delimiter=",")
